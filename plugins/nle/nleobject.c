...
nle_object_class_init (NleObjectClass * klass)
{
  ...
  properties[PROP_MEDIA_DURATION_FACTOR] =
      g_param_spec_float ("media-duration-factor", "Media duration factor",
      "The relative rate caused by this object", 0.01f, G_MAXFLOAT,
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MEDIA_DURATION_FACTOR:
      nleobject->media_duration_factor = g_value_get_float (value);
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_EXPANDABLE:
      ...
      break;
    case PROP_MEDIA_DURATION_FACTOR:
      g_value_set_float (value, nleobject->media_duration_factor);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
